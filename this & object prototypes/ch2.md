...
***There is no scope in the object, so neigher function or property is owned or contained by the object***
...
Whatever you choose to call this pattern, at the point that `foo()` is called, it's preceded by an object reference to `obj`. When there is a context object for a function reference, the

***implicit binding rule says that it's *that* object which should be used for the function call's `this` binding***.
